...
export function createEditorUtils(editor: IStandaloneCodeEditor) {
  return {
    ...
    wrapText(before: string, after = '') {
      ...
      if (fromBefore === before && toAfter === after) {
        const startColumn = range.startColumn - before.length,
          endColumn = range.endColumn + after.length
        ...
        editor.setSelection(new monaco.Selection(lineNumber, startColumn, lineNumber, endColumn))
      } else {
    ...
    replaceLines(replaceMethod: (line: string) => string) {
      ...
      editor.setSelection(
        new monaco.Selection(position.lineNumber, 1, position.lineNumber, newLineValue.length + 1)
      )
    },
    // 新增区块
